...
念念是一个**个人数字镜像 Agent**（Personal Digital Twin）。它通过日记、聊天、消费记录、书影音数据等多源信息，持续构建用户的深度人格画像、知识图谱和长期记忆，让 LLM 真正「认识」并「理解」你。
...
念念可以导入多种外部数据源，让记忆更完整：

| 数据源 | 导入方式 | 提取内容 |
|--------|---------|---------|
| 微信聊天 | 结构化数据导入 | 互动频率、情感倾向、回复模式、话题分布、语言风格 |
| 支付宝账单 | CSV 文件导入 | 消费模式、支出分类、商家偏好、消费趋势 |
| 书影音 | 结构化数据导入 | 题材偏好、评分分布、消费频率、情绪关联 |
| 社交媒体 | 结构化数据导入 | 社交互动、内容偏好、发布频率 |

微信导入支持**三级隐私控制**（Tier1 仅统计/Tier2 内容分析/Tier3 深度分析），Tier2 数据使用 Fernet 加密存储，Tier3 数据 24 小时自动过期。
